...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      shippingAddress, // { firstName, lastName, street1, street2, city, state, postalCode, phone, pathaoCityId, pathaoZoneId, pathaoAreaId }
    ...
    const orderItems: { productId: string; variantId?: string; name: string; sku: string; price: Decimal; quantity: number; total: Decimal }[] = [];
    ...
    for (const item of items) {
      ...
      const itemPrice = new Decimal(item.price || product.price);
      ...
      // If product out of stock → add to shortlist
