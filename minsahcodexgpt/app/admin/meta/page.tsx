...
export default function MetaOperationsCenterPage() {
  ...
  return <div className="space-y-6 p-4 md:p-6">
    <header className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
      <div>
        <div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-bold text-[#F7F8F8]">Meta Operations Center</h1><Badge tone="info">Phase 13</Badge></div>
        <p className="mt-1 max-w-3xl text-sm text-[#8a8f98]">Read-only Ads Insights, approval-based ad automation, deterministic product sets, first-party attribution, diagnostics, incidents, approvals and immutable audits in one redacted control plane.</p>
      </div>
    ...
    {message && <div className="rounded-xl border bg-[#161824] px-4 py-3 text-sm" role="status">{message}</div>}

    <nav className="flex gap-2 overflow-x-auto rounded-2xl border bg-[#161824] p-2" aria-label="Meta operations sections">
      {TABS.map((item) => <button key={item.id} onClick={() => setTab(item.id)} className={`min-h-11 shrink-0 rounded-xl px-3 text-sm font-semibold ${tab === item.id ? 'bg-gray-900 text-white' : 'text-[#8a8f98] hover:bg-[#10121b]'}`}>{item.label}</button>)}
    </nav>

    {!summary && <section className="rounded-2xl border bg-[#161824] p-8 text-center text-sm text-[#8a8f98]">{busy ? 'Loading Meta operational state…' : 'No operational snapshot available.'}</section>}
    ...
    {summary && tab === 'catalog' && <section className="space-y-4"><Panel title="Catalog final-state monitor" icon={Database}>
      ...
      <p className="text-sm text-[#8a8f98]">Submitted and delete-submitted items remain pending; they are not counted as successful until Meta returns a final state.</p>
...
function AdsPanel({ report, canOperate, busy, onSync, onGenerate, onDismiss, onRequestMutation }: {
  ...
  if (!report) return <section><Panel title="Ads Insights & approval automation" icon={BarChart3}><p className="text-sm text-[#8a8f98]">No persisted Ads Insights snapshot is available.</p>{canOperate && <Button disabled={busy} onClick={onSync}>Run first read-only sync</Button>}</Panel></section>;
  return <section className="space-y-4">
    ...
    <Panel title="Optimization recommendations — never auto-applied" icon={SearchCheck}>
      {report.recommendations.length === 0 ? <p className="text-sm text-[#8a8f98]">No active recommendations.</p> : <div className="space-y-3">{report.recommendations.map((item) => <article key={item.id} className="rounded-xl border p-4"><div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between"><div><div className="flex flex-wrap gap-2"><strong>{item.entityName ?? item.entityId}</strong><Badge tone={tone(item.severity)}>{item.severity}</Badge><Badge>{item.type}</Badge></div><p className="mt-2 text-sm text-[#d0d6e0]">{item.rationale}</p><p className="mt-1 text-xs text-[#8a8f98]">{item.entityType} · {item.entityId} · expires {date(item.expiresAt)}</p></div>{canOperate && <Button variant="secondary" disabled={busy} onClick={() => onDismiss(item.id)}>Dismiss</Button>}</div></article>)}</div>}
    </Panel>
    <Panel title="Persisted campaign snapshots" icon={BarChart3}>
      {report.snapshots.length === 0 ? <p className="text-sm text-[#8a8f98]">No snapshot rows.</p> : <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="border-b text-left"><th className="p-2">Campaign</th><th className="p-2">Spend</th><th className="p-2">Purchases</th><th className="p-2">ROAS</th><th className="p-2">CTR</th><th className="p-2">Window</th></tr></thead><tbody>{report.snapshots.slice(0, 100).map((row) => <tr key={row.id} className="border-b"><td className="p-2"><strong>{row.entityName ?? row.entityId}</strong><div className="text-xs text-[#8a8f98]">{row.entityId}</div></td><td className="p-2">৳{row.spend.toLocaleString('en-BD')}</td><td className="p-2">{row.purchases}</td><td className="p-2">{row.roas.toFixed(2)}</td><td className="p-2">{row.ctr.toFixed(2)}%</td><td className="p-2 text-xs">{new Date(row.dateStart).toLocaleDateString('en-BD')}–{new Date(row.dateStop).toLocaleDateString('en-BD')}</td></tr>)}</tbody></table></div>}
    </Panel>
    <Panel title="Approved mutation reconciliation ledger" icon={FileClock}>
      {report.executions.length === 0 ? <p className="text-sm text-[#8a8f98]">No approved ad mutation has been executed.</p> : <div className="space-y-2">{report.executions.map((item) => <div key={item.id} className="flex flex-col gap-1 rounded-xl border p-3 md:flex-row md:items-center md:justify-between"><div><strong className="text-sm">{item.operation}</strong><p className="text-xs text-[#8a8f98]">{item.entityType} · {item.entityId ?? 'provider ID pending'}</p></div><div className="flex items-center gap-2"><Badge tone={tone(item.status)}>{item.status}</Badge><span className="text-xs text-[#8a8f98]">{date(item.startedAt)}</span></div></div>)}</div>}
    </Panel>
  </section>;
}
...
function Overview({ summary, pendingApprovals }: { summary: Summary; pendingApprovals: number }) {
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-2"><Panel title="Provider state policy" icon={CheckCircle2}><p className="text-sm text-[#8a8f98]">Submission acceptance is displayed as pending. Only terminal provider states are reported as final success or failure.</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><KeyValue label="Catalog pending" value={count(summary.domains.catalog.items, 'SUBMITTED', 'DELETE_SUBMITTED')} /><KeyValue label="Events pending" value={count(summary.domains.events, 'PENDING', 'DISPATCHED', 'PROCESSING', 'RETRY_SCHEDULED')} /><KeyValue label="Jobs pending" value={count(summary.domains.jobs, 'QUEUED', 'RUNNING', 'RETRYING')} /></div></Panel><Panel title="Recent immutable actions" icon={FileClock}><AuditRows audits={summary.recentAudits.slice(0, 6)} /></Panel></div>
  </div>;
}
...
function ProductSetsPanel({ productSets, canOperate, busy, onCreate, onPreview, onRequestSync, onRollback }: {
  ...
  return <section className="space-y-4">
    ...
    <Panel title="Product sets and synchronized membership" icon={Database}>
      {productSets.length === 0 ? <p className="text-sm text-[#8a8f98]">No deterministic product sets exist.</p> : <div className="space-y-3">{productSets.map((item) => {
        const preview = item.previews[0];
        ...
        return <article key={item.id} className="rounded-xl border p-4"><div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between"><div><div className="flex flex-wrap items-center gap-2"><strong>{item.name}</strong><Badge tone={tone(item.status)}>{item.status}</Badge><Badge tone={tone(item.syncStatus)}>{item.syncStatus}</Badge>{item.autoSync && <Badge tone="info">AUTO</Badge>}</div><p className="mt-1 text-xs text-[#8a8f98]">v{item.ruleVersion} · {item.memberCount} member(s) · catalog {item.catalogId}</p><p className="mt-2 break-all text-xs text-[#8a8f98]">Rule {item.ruleHash.slice(0, 12)}… · membership {item.membershipHash?.slice(0, 12) ?? 'not previewed'}…</p><p className="mt-2 text-sm text-[#d0d6e0]">{item.ruleJson.combinator}: {item.ruleJson.conditions.map((condition) => `${condition.field} ${condition.operator} ${Array.isArray(condition.value) ? condition.value.join(', ') : String(condition.value)}`).join(' · ')}</p>{preview && <p className="mt-2 text-xs text-[#8a8f98]">Preview {preview.memberCount} members · expires {date(preview.expiresAt)}{preview.consumedAt ? ' · consumed' : ''}</p>}</div>{canOperate && <div className="flex flex-wrap gap-2"><Button variant="secondary" disabled={busy} onClick={() => onPreview(item)}>Preview</Button><Button disabled={busy || !previewFresh || item.memberCount === 0} onClick={() => onRequestSync(item)}>Request sync approval</Button>{previous && <Button variant="secondary" disabled={busy} onClick={() => onRollback(item, previous.version)}>Rollback to v{previous.version}</Button>}</div>}</div></article>;
      })}</div>}
    </Panel>
  </section>;
}
...
function DiagnosticsPanel({ diagnostics, canOperate, busy, onImport }: { diagnostics: Diagnostic[]; canOperate: boolean; busy: boolean; onImport: () => void }) {
  return <section><Panel title="Catalog Diagnostics" icon={SearchCheck}>
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><p className="text-sm text-[#8a8f98]">Persisted Meta diagnostic issues with sampled affected retailer IDs and correlation context.</p>{canOperate && <Button disabled={busy} onClick={onImport}><RefreshCw className="h-4 w-4" /> Import diagnostics</Button>}</div>
    {diagnostics.length === 0 ? <p className="text-sm text-[#8a8f98]">No active Catalog Diagnostics issues.</p> : <div className="space-y-3">{diagnostics.map((item) => <article key={item.id} className="rounded-xl border p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><strong>{item.title}</strong><Badge tone={tone(item.severity)}>{item.severity}</Badge><Badge tone={tone(item.status)}>{item.status}</Badge></div><p className="mt-1 text-xs text-[#8a8f98]">{item.issueType} · catalog {item.catalogId} · affected {item.affectedItemCount} · last seen {date(item.lastSeenAt)}</p>{item.description && <p className="mt-2 text-sm text-[#d0d6e0]">{item.description}</p>}{item.correlationId && <p className="mt-2 break-all text-xs text-[#8a8f98]">Correlation: {item.correlationId}</p>}</div></div>{item.items.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{item.items.map((affected) => <Badge key={affected.id}>{affected.retailerId}</Badge>)}</div>}</article>)}</div>}
  </Panel></section>;
}
...
function IncidentsPanel({ incidents, canOperate, busy, onAction }: { incidents: Incident[]; canOperate: boolean; busy: boolean; onAction: (incident: Incident, action: 'acknowledge' | 'resolve') => void }) {
  return <section><Panel title="Incident inbox" icon={Siren}>{incidents.length === 0 ? <p className="text-sm text-[#8a8f98]">No incidents have been persisted.</p> : <div className="space-y-3">{incidents.map((item) => <article key={item.id} className="rounded-xl border p-4"><div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between"><div><div className="flex flex-wrap items-center gap-2"><strong>{item.summary}</strong><Badge tone={tone(item.severity)}>{item.severity}</Badge><Badge tone={tone(item.status)}>{item.status}</Badge></div><p className="mt-1 text-xs text-[#8a8f98]">{item.incidentType} · {item.resourceType} · {item.resourceId ?? 'global'} · occurrences {item.occurrenceCount}</p><p className="mt-2 text-xs text-[#8a8f98]">First {date(item.firstSeenAt)} · Last {date(item.lastSeenAt)}</p>{item.correlationId && <p className="mt-2 break-all text-xs text-[#8a8f98]">Correlation: {item.correlationId}</p>}</div>{canOperate && item.status !== 'RESOLVED' && <div className="flex flex-wrap gap-2">{item.status === 'OPEN' && <Button variant="secondary" disabled={busy} onClick={() => onAction(item, 'acknowledge')}>Acknowledge</Button>}<Button disabled={busy} onClick={() => onAction(item, 'resolve')}>Resolve</Button></div>}</div></article>)}</div>}</Panel></section>;
}
...
function TracePanel({ query, trace, busy, onQuery, onLoad }: { query: string; trace: TraceResult | null; busy: boolean; onQuery: (value: string) => void; onLoad: () => void }) {
  return <section><Panel title="Correlation timeline" icon={Activity}>
    ...
    {!trace ? <p className="text-sm text-[#8a8f98]">Search a correlation ID to follow admin, queue, CAPI, webhook, catalog, diagnostic and incident events.</p> : <div><p className="mb-3 break-all text-xs text-[#8a8f98]">{trace.correlationId} · {trace.eventCount} event(s)</p>{trace.events.length === 0 ? <p className="text-sm text-[#8a8f98]">No persisted events matched this correlation ID.</p> : <ol className="space-y-3">{trace.events.map((item, index) => <li key={`${item.source}-${item.resourceId}-${index}`} className="rounded-xl border p-4"><div className="flex flex-wrap items-center gap-2"><Badge>{item.source}</Badge><Badge tone={tone(item.status)}>{item.status}</Badge><span className="text-xs text-[#8a8f98]">{date(item.occurredAt)}</span></div><p className="mt-2 text-sm font-semibold">{item.summary}</p><p className="mt-1 break-all text-xs text-[#8a8f98]">{item.resourceId}</p></li>)}</ol>}</div>}
  </Panel></section>;
}
...
function FailurePanel({ title, items, canOperate, busy, onRequest }: { title: string; items: FailureItem[]; canOperate: boolean; busy: boolean; onRequest: (item: FailureItem) => void }) {
  return <section><Panel title={title} icon={AlertTriangle}>{items.length === 0 ? <p className="text-sm text-[#8a8f98]">No matching failures.</p> : <div className="space-y-3">{items.map((item) => <article key={item.id} className="rounded-xl border p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><strong>{item.eventName ?? item.jobName ?? item.id}</strong><Badge tone={tone(item.status)}>{item.status}</Badge></div><p className="mt-1 text-xs text-[#8a8f98]">{item.eventId ?? item.queueName ?? item.id} · attempts {item.attempts} · {date(item.updatedAt)}</p><p className="mt-2 text-sm text-[#d0d6e0]">{adminFailureText(item.failure)}</p></div>{canOperate && <Button variant="secondary" disabled={busy} onClick={() => onRequest(item)}>Request replay approval</Button>}</div></article>)}</div>}</Panel></section>;
}
...
function ApprovalsPanel({ approvals, currentUserId, canApprove, canOperate, busy, onDecide, onExecute }: { approvals: Approval[]; currentUserId: string; canApprove: boolean; canOperate: boolean; busy: boolean; onDecide: (item: Approval, decision: 'approve' | 'reject') => void; onExecute: (item: Approval) => void }) {
  return <section><Panel title="Approval queue" icon={ShieldCheck}>{approvals.length === 0 ? <p className="text-sm text-[#8a8f98]">No approval requests.</p> : <div className="space-y-3">{approvals.map((item) => <article key={item.id} className="rounded-xl border p-4"><div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between"><div><div className="flex flex-wrap gap-2"><strong>{item.actionKey}</strong><Badge tone={tone(item.status)}>{item.status}</Badge><Badge tone={item.risk === 'CRITICAL' || item.risk === 'HIGH' ? 'danger' : 'warning'}>{item.risk}</Badge></div><p className="mt-1 text-xs text-[#8a8f98]">{item.resourceType} · {item.resourceId ?? 'no resource ID'} · requested by {item.requestedBy.name}</p><p className="mt-2 text-sm text-[#d0d6e0]">{item.reason}</p><p className="mt-1 text-xs text-[#8a8f98]">Expires {date(item.expiresAt)}</p></div><div className="flex flex-wrap gap-2">{item.status === 'PENDING' && canApprove && item.requestedBy.id !== currentUserId && <><Button disabled={busy} onClick={() => onDecide(item, 'approve')}>Approve</Button><Button variant="danger" disabled={busy} onClick={() => onDecide(item, 'reject')}>Reject</Button></>}{item.status === 'APPROVED' && canOperate && <Button disabled={busy} onClick={() => onExecute(item)}>Execute approved action</Button>}</div></div></article>)}</div>}</Panel></section>;
}
  ...
  return <section className="space-y-4">
    <Panel title="First-party attribution & growth analytics" icon={BarChart3}>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border p-4"><div className="flex items-center gap-2"><strong>{report?.models.firstParty.label ?? 'First-party attribution'}</strong><Badge tone="info">INTERNAL</Badge></div><p className="mt-2 text-sm text-[#8a8f98]">Immutable first-touch, eligible last-touch, lead inheritance and checkout snapshots.</p></div>
        <div className="rounded-xl border p-4"><div className="flex items-center gap-2"><strong>{report?.models.metaReported.label ?? 'Meta-reported attribution'}</strong><Badge>PROVIDER</Badge></div><p className="mt-2 text-sm text-[#8a8f98]">{report?.models.metaReported.availability ?? 'Open Meta Insights separately; provider attribution is not combined with first-party totals.'}</p></div>
      </div>
    </Panel>
    <Panel title="Campaign → sessions, leads, orders and revenue" icon={BarChart3}>
      {campaigns.length === 0 ? <p className="text-sm text-[#8a8f98]">No first-party campaign aggregates are available for this window.</p> : <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="border-b text-left"><th className="p-2">Campaign</th><th className="p-2">Source / medium</th><th className="p-2">Sessions</th><th className="p-2">Leads</th><th className="p-2">Orders</th><th className="p-2">Revenue</th></tr></thead><tbody>{campaigns.map((row) => <tr key={`${row.utmSource}-${row.utmMedium}-${row.utmCampaign}`} className="border-b"><td className="p-2 font-medium">{row.utmCampaign}</td><td className="p-2">{row.utmSource} / {row.utmMedium}</td><td className="p-2">{row.sessions}</td><td className="p-2">{row.leads}</td><td className="p-2">{row.orders}</td><td className="p-2">৳{row.revenue.toLocaleString('en-BD')}</td></tr>)}</tbody></table></div>}
    </Panel>
...
function AuditRows({ audits }: { audits: Audit[] }) { return audits.length === 0 ? <p className="text-sm text-[#8a8f98]">No audit records.</p> : <div className="divide-y">{audits.map((item) => <div key={item.id} className="flex flex-col gap-2 py-3 md:flex-row md:items-center md:justify-between"><div><div className="flex flex-wrap gap-2"><strong className="text-sm">{item.actionKey}</strong><Badge tone={tone(item.outcome)}>{item.outcome}</Badge><Badge>{item.risk}</Badge></div><p className="mt-1 text-xs text-[#8a8f98]">{item.resourceType} · {item.resourceId ?? '—'} · {item.actor.name}</p></div><span className="text-xs text-[#8a8f98]">{date(item.createdAt)}</span></div>)}</div>; }
function StatusGrid({ counts }: { counts: Counts }) { const entries = Object.entries(counts); return entries.length === 0 ? <p className="text-sm text-[#8a8f98]">No persisted records.</p> : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{entries.map(([status, value]) => <div key={status} className="rounded-xl border p-3"><Badge tone={tone(status)}>{status}</Badge><div className="mt-2 text-2xl font-bold">{value}</div></div>)}</div>; }
function Panel({ title, icon: Icon, children }: { title: string; icon: typeof Activity; children: React.ReactNode }) { return <section className="rounded-2xl border bg-[#161824] p-5"><h2 className="mb-4 flex items-center gap-2 text-lg font-bold"><Icon className="h-5 w-5 text-[#8a8f98]" />{title}</h2><div className="space-y-4">{children}</div></section>; }
function Metric({ icon: Icon, label, value, status }: { icon: typeof Activity; label: string; value: string | number; status: string }) { return <div className="rounded-2xl border bg-[#161824] p-4"><Icon className="h-5 w-5 text-[#8a8f98]" /><div className="mt-3 text-xl font-bold">{value}</div><div className="mt-1 text-sm text-[#8a8f98]">{label}</div><Badge className="mt-3" tone={tone(status)}>{status}</Badge></div>; }
function KeyValue({ label, value, badge = false }: { label: string; value: string | number; badge?: boolean }) { return <div className="rounded-xl border p-3"><div className="text-xs font-medium uppercase tracking-wide text-[#8a8f98]">{label}</div><div className="mt-2 font-semibold">{badge ? <Badge tone={tone(String(value))}>{String(value)}</Badge> : value}</div></div>; }
function Callout({ children }: { children: React.ReactNode }) { return <div className="rounded-xl border border-amber-500/20 bg-amber-500/10 p-4 text-sm text-amber-900">{children}</div>; }
