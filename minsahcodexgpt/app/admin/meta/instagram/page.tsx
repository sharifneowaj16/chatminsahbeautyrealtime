...
export default function InstagramSocialCrmPage() {
  ...
  return <main className="mx-auto max-w-7xl space-y-5 p-4 md:p-6">
    <header className="flex flex-wrap items-start justify-between gap-3">
      <div>
        <Link href="/admin/meta" className="mb-2 inline-flex items-center gap-2 text-sm text-[#8a8f98] hover:text-[#F7F8F8]"><ArrowLeft className="h-4 w-4" /> Meta Operations Center</Link>
        <h1 className="flex items-center gap-2 text-2xl font-bold"><MessageCircle className="h-6 w-6" /> Instagram Social CRM</h1>
        <p className="mt-1 text-sm text-[#8a8f98]">Webhook, queue, permission, delivery, reconciliation and reply-policy state from the durable platform.</p>
      </div>
      <Button disabled={busy} onClick={() => void load()}><RefreshCw className="mr-2 h-4 w-4" /> Refresh</Button>
    </header>

    {health && <>
      <section className="grid gap-3 rounded-2xl border bg-[#161824] p-4 md:grid-cols-4">
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-2xl border bg-[#161824] p-4">
        ...
        <div className="rounded-2xl border bg-[#161824] p-4">
          ...
          {health.failure && <p className="mt-3 rounded-xl border border-rose-500/20 bg-rose-500/10 p-3 text-sm text-rose-300">{failureText(health.failure)}</p>}
        </div>
      </section>

      <section className="overflow-hidden rounded-2xl border bg-[#161824]">
        ...
        {health.deadLetters.length === 0 ? <p className="p-4 text-sm text-[#8a8f98]">No Instagram dead-letter or failed jobs.</p> : <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="bg-[#10121b]"><tr>{['Job','State','Attempts','Next run','Safe failure'].map((heading) => <th key={heading} className="px-4 py-3 text-left">{heading}</th>)}</tr></thead><tbody className="divide-y">{health.deadLetters.map((job) => <tr key={job.id}><td className="px-4 py-3"><strong>{job.jobName}</strong><div className="text-xs text-[#8a8f98]">{job.queueName}</div></td><td className="px-4 py-3"><Badge tone={tone(job.status)}>{job.status}</Badge></td><td className="px-4 py-3">{job.attempts}/{job.maxAttempts}</td><td className="px-4 py-3">{date(job.nextRunAt)}</td><td className="max-w-lg px-4 py-3">{failureText(job.failure)}</td></tr>)}</tbody></table></div>}
      </section>
    </>}

    {notice && <div className="rounded-xl border bg-amber-500/10 px-4 py-3 text-sm text-amber-900" role="status">{notice}</div>}

    <section className="grid gap-5 lg:grid-cols-[380px_1fr]">
      <aside className="rounded-2xl border bg-[#161824]">
        ...
        <div className="max-h-[70vh] overflow-y-auto">
          {items.map((item) => <button key={item.id} onClick={() => void openConversation(item.id)} className={`w-full border-b p-4 text-left hover:bg-[#10121b] ${selected?.id === item.id ? 'bg-[#10121b]' : ''}`}>
            ...
            <p className="mt-1 line-clamp-2 text-xs text-[#8a8f98]">{item.messages?.[0]?.text || item.messages?.[0]?.messageType || 'No message preview'}</p>
            ...
            <p className="mt-2 text-[11px] text-[#62666d]">{date(item.lastActivityAt ?? item.lastMessageAt)}</p>
          </button>)}
          {items.length === 0 && <p className="p-6 text-sm text-[#8a8f98]">No conversations found.</p>}
      ...
      <section className="min-h-[60vh] rounded-2xl border bg-[#161824] p-4 md:p-5">
        {!selected && <div className="flex min-h-[50vh] items-center justify-center text-sm text-[#8a8f98]">Select a conversation.</div>}
        {selected && <div className="space-y-5">
          <div className="flex flex-wrap items-start justify-between gap-3 border-b pb-4">
            <div><h2 className="text-lg font-bold">@{selected.participantUsername || selected.participantName || selected.participantId}</h2><p className="text-xs text-[#8a8f98]">{selected.platformId}</p></div>
          ...
          <div className="max-h-[45vh] space-y-3 overflow-y-auto rounded-xl bg-[#10121b] p-3">
            {selected.messages.map((message) => <article key={message.id} className={`max-w-[90%] rounded-2xl p-3 text-sm ${message.direction === 'OUTBOUND' ? 'ml-auto bg-gray-900 text-white' : 'bg-[#161824] shadow-sm'}`}>
          ...
          {selected.replyAttempts.length > 0 && <section className="rounded-xl border p-3"><h3 className="mb-2 font-semibold">Recent reply attempts</h3><div className="space-y-2">{selected.replyAttempts.map((attempt) => <div key={attempt.id} className="rounded-lg bg-[#10121b] p-2 text-xs"><div className="flex flex-wrap gap-2"><Badge tone={tone(attempt.status)}>{attempt.status}</Badge><Badge>{attempt.mode}</Badge><span>Provider: {attempt.providerStatus}</span><span>Reconciliation: {attempt.reconciliationStatus}</span></div>{attempt.failure && <p className="mt-1 text-rose-400">{failureText(attempt.failure)}</p>}</div>)}</div></section>}
          ...
          {canOperate && <div className="space-y-3 rounded-xl border p-3">
            ...
            {!health?.replyControl.standard.enabled && <p className="text-xs text-amber-400">Standard write control: {health?.replyControl.standard.reasonCode}</p>}
            {!health?.replyControl.private.enabled && <p className="text-xs text-amber-400">Private write control: {health?.replyControl.private.reasonCode}</p>}
            {!selected.replyEligibility.standard.allowed && <p className="text-xs text-amber-400">Standard reply blocked: {selected.replyEligibility.standard.reasonCode}</p>}
          </div>}
...
function HealthCard({ label, ok, value }: { label: string; ok: boolean; value: string }) {
  return <div className="rounded-xl border p-3">{ok ? <CheckCircle2 className="h-5 w-5 text-green-600" /> : <ShieldAlert className="h-5 w-5 text-amber-600" />}<p className="mt-2 text-xs text-[#8a8f98]">{label}</p><strong className="text-sm">{value}</strong></div>;
}

function StateGroup({ title, values }: { title: string; values: Record<string, number> }) {
  const entries = Object.entries(values);
  return <div className="rounded-xl border p-3"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#8a8f98]">{title}</p>{entries.length === 0 ? <p className="text-sm text-[#8a8f98]">No records</p> : <div className="flex flex-wrap gap-2">{entries.map(([status, count]) => <Badge key={status} tone={tone(status)}>{status}: {count}</Badge>)}</div>}</div>;
}

function KeyValue({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl border p-3"><div className="text-xs uppercase tracking-wide text-[#8a8f98]">{label}</div><div className="mt-1 break-words font-medium">{value}</div></div>;
}
