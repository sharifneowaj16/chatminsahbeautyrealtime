...
import { getMetaLeadConfig, requireMetaLeadEncryptionSecret, requireMetaLeadFingerprintSecret } from '@/lib/meta/leads/config';
import { fetchMetaLeadGraphRecord, MetaLeadFetchError } from '@/lib/meta/leads/fetch';
import { normalizeMetaLeadFields } from '@/lib/meta/leads/normalize';
import { notifyMetaLeadAssignment } from '@/lib/meta/leads/notify';
import {
  assignMetaLead,
  runMetaLeadRetentionCleanup,
  runMetaLeadSlaScan,
} from '@/lib/meta/leads/repository';
import {
...
import { executeMetaLeadCrmHandoff } from './process-lead';
import { resolveMetaTestLeadPolicy } from './test-lead';
import { evaluateMetaLeadFormOwnership } from './ownership-policy';
import {
  getMetaWebhookReceipt,
  listRecoverableMetaLeadReceipts,
  markMetaWebhookReceipt,
} from '@/lib/meta/leads/receipt';
...
export async function processMetaLeadReceipt(input: {
  ...
  try {
    ...
    const formOwnership = evaluateMetaLeadFormOwnership({
      notificationFormId,
      providerFormId: raw.form_id,
      allowedFormIds: config.allowedFormIds,
    });
    if (!formOwnership.ok) {
      throw new MetaLeadPermanentProcessingError(formOwnership.code, formOwnership.safeMessage);
    }
    const formId = formOwnership.formId;
    ...
    const testLeadPolicy = resolveMetaTestLeadPolicy({ providerMarker: raw.is_test_lead, source: 'PROVIDER_MARKER' });
    let assignment: { assignedToId: string | null; reason: string; ruleId: string | null };
    if (testLeadPolicy.isolateFromCrm) {
      await blockMetaLeadHandoff({
        handoffId: persisted.handoffId,
        reasonCode: 'META_TEST_LEAD_ISOLATED',
        reasonSummary: 'Test Lead is isolated from normal CRM handoff.',
      });
      assignment = { assignedToId: null, reason: 'TEST_LEAD_ISOLATED', ruleId: null };
    } else {
      const handoff = await executeMetaLeadCrmHandoff({
        handoffId: persisted.handoffId,
        leadId: persisted.leadId,
        claim: (handoffId) => claimMetaLeadHandoff({ handoffId, now: input.now }),
        run: ({ leadId }) => assignMetaLead(leadId),
        complete: completeMetaLeadHandoff,
        fail: ({ handoffId, failure, terminal }) => failMetaLeadHandoff({
          handoffId,
          failureCode: failure.code,
          failureSummary: failure.safeSummary,
          terminal,
          now: input.now,
        }),
      });
      assignment = handoff.value ?? { assignedToId: null, reason: handoff.status, ruleId: null };
    }
    await markMetaWebhookReceipt({ receiptId: receipt.id, status: 'PROCESSED' });
    ...
    if (!testLeadPolicy.suppressNotifications) {
    ...
    return { receiptId: receipt.id, ...persisted, assignment, testLeadPolicy, status: 'PROCESSED' as const };
  } catch (error) {
